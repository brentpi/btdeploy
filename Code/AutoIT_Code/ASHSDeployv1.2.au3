...
Func EraseDownloadApplyWIM($strName)
	...
	If (DriveGetLabel($drvSystem) <> "System") Or (DriveGetLabel($drvData) <> "Data") Then
		...
		MsgBox(0, "Error", "Failed Partition Check. Either System or Data isnt showing up in their correct locations. Exiting.")
		Return 0
	EndIf
...
;~ 	If $strName <> "CFS" Then
	If $strName == "CFS" Then
		FixBCDDrive(StringLeft($drvHome, 1), True)
		FixBCDDrive(StringLeft($drvSystem, 1), True)
	Else
		FixBCDDrive(StringLeft($drvSystem, 1), True)
	EndIf
;~ 		$outBootSect = RunWait("X:\Windows\System32\bootsect.exe /nt60 " & $drvHome & " /MBR /FORCE", "X:\", @SW_SHOWNORMAL)
;~ 		If ($outBootSect = 0) And (@error <> 0) Then
;~ 			MsgBox(0, "Error!", "Failed to fix boot sector")
;~ 			Return 0
;~ 		EndIf
;~ 		$outBootSect = RunWait("X:\Windows\System32\bootsect.exe /nt60 " & $drvSystem & " /MBR /FORCE", "X:\", @SW_SHOWNORMAL)
;~ 		If ($outBootSect = 0) And (@error <> 0) Then
;~ 			MsgBox(0, "Error!", "Failed to fix boot sector")
;~ 			Return 0
;~ 		EndIf
...
;~	EndIf
;~ 	Else
;~ 		; we make it XP compatible for grub4dos.
;~ 		$outBootSect = RunWait("X:\Windows\System32\bootsect.exe /nt " & $drvSystem & " /MBR /FORCE", "X:\", @SW_SHOWNORMAL)
;~ 		If ($outBootSect = 0) And (@error <> 0) Then
;~ 			MsgBox(0, "Error!", "Failed to fix boot sector")
;~ 			Return 0
;~ 		EndIf
...
;~ If FileExists($drvData & $strName & "\Image.mbr") Then
   		If $strName == "CFS" Then
			...
			EndIf
   		EndIf
;~    		; $outMBRFix = RunWait("X:\Program Files\MBRFix\MBRFix.exe /Restore=MBR /Filename=" & $drvData & $strName & "\Image.mbr /Confirm", "X:\", @SW_SHOWNORMAL)
;~    		$outMBRFix = RunWait("X:\Program Files\MBRFix\MBRFix.exe /drive 0 restorembr " & $drvData & $strName & "\Image.mbr /yes", "X:\", @SW_SHOWNORMAL)
;~    		If ($outMBRFix = 0) And (@error <> 0) Then
;~    			MsgBox(0, "Error!", "Failed to apply custom MBR.")
;~    			Return 0
;~    		EndIf
;~    	EndIf
;~ 	
	; Save this for later in case something doesnt work.
	; $outMBRActive = RunWait("X:\Program Files\MBRFix\MBRFix.exe /Disk=0 /Part=3 /Active", "X:\", @SW_SHOWNORMAL)
	
	If $strName <> "CFS" Then
		$outBCDBoot = RunWait("X:\Windows\System32\bcdboot.exe " & $drvSystem & "Windows /s " & StringLeft($drvSystem, 2), "X:\", @SW_SHOWNORMAL)
		If ($outBCDBoot = 0) And (@error <> 0) Then
			MsgBox(0, "Error!", "Failed to fix BCD")
			Return 0
		EndIf
	EndIf
...
Func FixBCDDrive($strDrive, $bFixSector)
	...
	If $bFixSector == True Then
		$outBootSect = RunWait("X:\Windows\System32\bootsect.exe /nt60 " & $strLetter & ":\ /MBR", "X:\", @SW_SHOWNORMAL)
